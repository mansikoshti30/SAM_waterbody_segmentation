...
OUTPUT_FOLDER = "batch_outputs_3"               # folder where outputs will be saved
...
def process_one_tif(tif_path, sam, predictor, out_folder):
    ...
    # choose index: MNDWI > NDWI > RGB fallback
    ...
    # prepare SAM crop predictor (sam model loaded outside)
    ...
    for idx, (x0, y0, x1, y1) in enumerate(boxes, start=1):
        ...
        if best_mask is None: 
            continue
        if best_iou < 0.05 and len(scores) > 0:
        ...
        print(f"Candidate {idx}/{len(boxes)} iou={best_iou:.3f}")
